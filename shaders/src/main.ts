...
import { GLObject, ModelObject } from "./object"
import { Shader, Shaders, init_webgl_shaders } from "./shaders"
import { DataLoader, Geometry } from "./data_loader"
import { MODEL_DATA } from "./model_data"
import { Camera } from "./camera"
import { Light } from "./light"

const PROJECTION_MATRIX = mv.perspective(60, 1, .1, 20);
const INVERSE_PROJECTION_MATRIX = mv.inverse4(PROJECTION_MATRIX);
const LIGHT_PROJECTION_MATRIX = mv.perspective(150, 1, .1, 20);
const SKYBOX = new Float32Array([-1, -1, 1, -1, -1, 1, 1, 1]);
const SHADOW_MULT = 4;

interface ImportantObj {
    cameras: Camera[],
    light: Light,
}

interface Track_Camera {
    is_moving: boolean,
    spin: number;
}
...
interface Model {
    ...
    import_obj: ImportantObj,
    objects: Record<string, GLObject>,
    data: DataLoader,
    light_type: number,
    ...
    skybox: Skybox;
    shadows: Shadows;
    show_reflect: boolean;
    show_refract: boolean;
}
...
async function main() {
    ...
    let gl = webgl.setup_webgl(boundary.canvas);
...
function make_gl_stuff(gl: WebGL2RenderingContext, canvas: HTMLCanvasElement): [Skybox, Shadows] {
    // skybox
...
function load_scene(): [Record<string, GLObject>, ImportantObj] {
    let important = {
        cameras: [],
        light: new Light(),
    }
    let scene = {};
    const add_to_scene = (obj: GLObject): GLObject => {
        scene[obj.name] = obj;
        return obj;
    }
    const add_camera = (name: string): Camera => {
        let ret = new Camera(name);
        important.cameras.push(ret);
        return ret;
    }

    add_to_scene(new GLObject("track_camera_anchor")
        .add_child(
            add_camera("Track")
                .set_translation(mv.vec3(0, 4, 8))
        ));
    let car = new ModelObject("car", "car_Cube.012")
        .set_translation(mv.vec3(3, 0, 0))
        .set_rotation(mv.vec3(0, 180, 0))
        .set_skybox_type(1)
        .add_child(new ModelObject("back_wheels", "NormalCar1_BackWheels_Cube.011"))
        .add_child(new ModelObject("front_left_wheel", "NormalCar1_FrontLeftWheel_Cube.007"))
        .add_child(new ModelObject("front_right_wheel", "NormalCar1_FrontRightWheel_Cube.008"))
        .add_child(new ModelObject("bunny", "bunny")
            .set_translation(mv.vec3(0, .5, 2))
            .set_rotation(mv.vec3(0, 90, 0))
            .set_skybox_type(-1)
        )
        .add_child(add_camera("Bumper")
            .set_translation(mv.vec3(0, 1, -3)))
        .add_child(new GLObject("1st Person Lookat")
            .set_translation(mv.vec3(.5, 1, 1))
            .add_child(add_camera("1st Person")
                .set_translation(mv.vec3(0, 0, -0.5))))
        .add_child(add_camera("3rd Person")
            .set_translation(mv.vec3(0, 3, -5)))

    add_to_scene(new ModelObject("lamp", "Streetlight_Single_Cylinder.003").set_no_shadow())
        .add_child(important.light.set_translation(mv.vec3(0, 2.95, 0)))
        .add_child(add_camera("Light")
            .set_translation(mv.vec3(0, 5, 0)).set_up_dir(mv.vec3(1, 0, 0)))

    add_to_scene(new ModelObject("street", "Street_Curve_Cube.004"));
    add_to_scene(new ModelObject("stopsign", "Stop_Cylinder"))
        .set_translation(mv.vec3(4.1, 0, -2))
        .set_rotation(mv.vec3(0, -90, 0));
    add_to_scene(new GLObject("car_anchor"))
        .add_child(car);
...
function render(model: Model) {
    ...
    for (let obj of Object.values(model.objects)) {
        obj.make_transform(mv.mat4());
    }

    let cameraMat = model.import_obj.cameras[model.camera_number].get_matrix();

    let light_mat = model.import_obj.light.get_matrix();
    let invCameraMat = mv.inverse4(cameraMat);
    let shadow_matrix = mv.mult_mat(LIGHT_PROJECTION_MATRIX, light_mat);

    let textureMatrix = mv.mult_mat(
        mv.mult_mat(
            mv.mult_mat(
                mv.translate(mv.vec3(.5, .5, .5)),
                mv.scalem(mv.vec3(.5, .5, .5)),
            ),
    ...
    let inverse_matrix = mv.mult_mat(invCameraMat, INVERSE_PROJECTION_MATRIX);
...
function render_shadowbuffer(model: Model, shadow_matrix: mv.Mat4) {
    let gl = model.gl;
...
function render_shadow_obj(model: Model, shader: Shader, obj: GLObject, matrix: mv.Mat4) {
    let gl = model.gl;
    ...
    if (obj instanceof ModelObject && obj.has_shadow) {
        ...
        let modelMat = mv.mult_mat(matrix, obj.get_transform());
        gl.uniformMatrix4fv(
            shader.uniforms['mMatrix'],
            true, new Float32Array(modelMat.flatMap(e => e)));
...
function render_skybox(model: Model, inverse_matrix: mv.Mat4) {
    let gl = model.gl;
    ...
    gl.uniformMatrix4fv(
        ...
        true, new Float32Array(inverse_matrix.flatMap(e => e)));
...
function render_objects(model: Model, cameraMat: mv.Mat4, shadow_matrix: mv.Mat4, inverse_matrix: mv.Mat4) {
    let gl = model.gl;
    ...
    gl.uniformMatrix4fv(
        ...
        true, new Float32Array(PROJECTION_MATRIX.flatMap(vec => vec)));

    gl.uniformMatrix4fv(
        shader.uniforms['mView'],
        true, new Float32Array(cameraMat.flatMap(vec => vec)));

    gl.uniformMatrix4fv(
        shader.uniforms['mShadow'],
        true, new Float32Array(shadow_matrix.flatMap(vec => vec)));

    gl.uniformMatrix4fv(
        shader.uniforms['mInverseView'],
        true, new Float32Array(inverse_matrix.flatMap(vec => vec)));

    gl.uniform1f(
        shader.uniforms['light_type'],
        1 - model.light_type);

    let light_pos = mv.vec3(mv.mult_mat_vec(cameraMat,
...
function render_obj(model: Model, shader: Shader, obj: GLObject) {
    let gl = model.gl;
    ...
    if (obj instanceof ModelObject) {
        ...
        let skybox_type = mv.clamp(obj.skybox_type, -Number(model.show_refract), Number(model.show_reflect))
...
function render_geometry(model: Model, shader: Shader, geo: Geometry) {
    let gl = model.gl;
    ...
    gl.uniformMatrix3fv(
        ...
        false, new Float32Array(materal_data.flatMap(vec => vec)));
...
function update(model: Model) {
    ...
    if (model.track_camera.is_moving) {
        ...
        model.objects["track_camera_anchor"].set_rotation(mv.vec3(0, model.track_camera.spin, 0));
        model.import_obj.cameras[0].translation[1] = 4 + Math.sin(model.track_camera.spin / 32);
    }
    let car_acc = model.car.is_moving ? 1 : -1;
    model.car.speed = mv.clamp(model.car.speed + car_acc / 4, 0, 5);
...
function setup_controls(model: Model) {
